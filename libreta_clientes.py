...
def nuevo_cliente():
    ...
    def guardar():
        if not nombre.get():
            ...
            return #corta la ejecución del resto de la funcion
        if not telefono.get():
        ...
        insertar(cliente)
        #cierro la ventana top (nuevo cliente)
        top.destroy()
...
def eliminar_cliente():
    id = tree.selection()[0] #tomo el primer elemento seleccionado
    cliente = c.execute("SELECT * FROM cliente WHERE id = ?", (id, )).fetchone()

    respuesta = messagebox.askokcancel('Seguro?', 'Eliminar ' + cliente[1] + '?')
    if respuesta:
